function CVerr = cvglmnet(x,y,family,options,type,nfolds,foldid,parallel,keep,grouped,repeats,reweight)

% CHANGES BY JUSSI TOHKA 22.3.2020:
%  - repeats of the cross-validation
%  - incorporating AUC computation by pooling, 
...
if nargin < 11 || isempty(repeats)
    repeats = 1;
end
if nargin < 12 || isempty(reweight)
    reweight = 0;
end
...
if repeats < 2
    ...
    for i = 1:repeats
        foldid{i} = reshape(foldid{i}, numel(foldid{i}), 1);
    end
end
...
cpredmat = cell(repeats,nfolds,1);
uy = unique(y);
    ...
    for k = 1: repeats
        for i = 1: nfolds        
            ...
            if reweight
                 ...
                 weights(yr == uuy(1)) = 1/sum(yr == uuy(1));% re-weighting the data
                 weights(yr == uuy(2)) = 1/sum(yr == uuy(2));
                 weights = weights*length(yr)/2;
                 opts.weights = weights;
            end
            cpredmat{k,i} = glmnet(xr, yr, family, opts);
        end
    end
end
...
switch cpredmat{1}.class
    ...
    case 'lognet'
        cvstuff = cvlognet_jt(cpredmat,options.lambda,x,y,options.weights,options.offset,foldid,type,grouped,keep,repeats);
    case 'multnet'
